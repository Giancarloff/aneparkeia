...
\subsection{A step back}

It seems more prudent trying to understand the endomorphism set in its usual form rather then trying some form of group completion. I like the name endomorphism but I will allow myself to also use the name "function set" to describe it.

So we know constant functions of the form $f(x) = a$ are right-zeros in the product, in the sense they determine the whole of the product as themselves by simply being a factor in it at the very end.

Well, there are as many right-zeroes as there are elements of the set, because for each $c \in S$ we can separate the function $f(x) = s$ and count it as one. The total number of functions that exist in a set $S$ to itself is known to be, for $n = |S|$, $n^n$. It is also known that there are $n!$ bijections and $n$ right-zeroes. So we got a nice little sum to break down \begin{equation*}
    n^n = n! + n + R
\end{equation*}
where $R$ is hiding the other amounts of types of function.

We could elaborate on all the summands hidden in $R$ through combinatorics. For that, let us say that a function is an $k$-classification if its image has cardinality $k$ (I think of it as the function classifying the set into $k$ classes). Let us restrict ourselves to finite sets for now. We know that there are $n!$ $n$-classifications and $n$ $1$-classifications. 
...
\begin{note}
    We will write $(a, b) \in R$ as $b | R = a$. This can be read as $b$ comes from $a$ by $R$. The second condition for branching tells us this notation is well defined.

    We will also write a branching $R \subseteq A \times B$ as $R : B \ot A$. One way to read this is: by $R$, what is in $B$ comes from $A$. A better way is $A$ branches from $B$, which I like more because it follows the arrow.
\end{note}
