...
\newcommand{\automataAs}[1]{#1 : A \to \funcset{[s]}}
...
Uma questão inicial bem simples de se fazer seria: Se sabemos que variando $n \in [s]$, todas as nossas linguagens são translações uma das outras por palavras fixas, conseguimos determinar algo sobre $F$ e $\phi$? Se $s = 3$, por exemplo, e digamos que \begin{equation}
    L(\phi, 1, F) = aL(\phi, 2, F) = abL(\phi, 3, F),
\end{equation}
...
Claramente a definição de autômato induz uma ação do monóide sobre o conjunto de funções. Para uma palavra $w = a_0a_1...a_n$ em $\ainf$, obtemos a ação de $w$ em $x \in \funcset{[s]}$ por \begin{equation}
    wx = \phi(x | a_0a_1...a_n)
\end{equation}
...
\begin{proplivre} \label{prop:composicao_automato}
    Seja $\automataAs{\phi}$ um autômato, $x \in [s]$ e $a, b \in A$. Temos que \begin{equation}
...
\begin{nota}
    Sejam $\automataAs{\phi}$ um autômato, $i \in [s]$, e $p \in \ainf$. Escrevendo $p$ por extenso como \begin{equation}
        p = (p_1, p_2, ..., p_k)
    \end{equation}
    extendemos nossa definição de $\phi(x | y)$ fazendo \begin{equation}
        \phi(i | p) = \phi(i | p_1p_2...p_k).
    \end{equation}
    No caso $p = \varepsilon$, convencionaremos que $\phi(i | \varepsilon) = i$.
\end{nota}

\begin{nota}
    Denotaremos por $Z(\phi)$ o conjunto \begin{equation}
        Z(\phi) = \set{a \in A : \phi(a) = id},
    \end{equation}
    por $A \leq B$ o fato de que $A$ é um subgrupo de $B$, e por $P_s$ o grupo de permutações (funções bijetoras) de $[s] \to [s]$. Imediatamente temos que $P_s \subseteq \funcset{[s]}$.
\end{nota}

\begin{proposicao}[Autômato Subgrupo] \label{prop:automato_subgrupo}
    Seja $\automataAs{\phi}$ um autômato e $P_s$ o grupo de permutação sobre $[s]$. Se $\phi(A) \leq P_s$, então valem: \begin{enumerate}
        \item Para todo $n \in [s]$, existem $a, b \in A$ tais que $\phi(n | ab) = n$;
        \item Para todo $a \in Z(\phi)$ e $n \in [s]$, temos $\phi(n | a) = n$; e
        \item Para todo $n \in [s]$ e $a, b \in A$, existe $c \in A$ tal que $\phi(n | ab) = \phi(n | c)$.
    \end{enumerate}
\end{proposicao}

\begin{demonstracao}
    Para (1), tome $a \in A$ e denote por $\sigma_a$ a permutação $\phi(a)$. Como $\phi(A) \leq P_s$, temos que $\sigma_a^{-1} \in \phi(A)$, ou seja, existe $b \in A$ tal que $\phi(b) = \sigma_a^{-1}$. Assim, para todo $n \in [s]$, temos \begin{equation}
        \phi(n | ab) = (\phi(b) \circ \phi(a))(n) = (\sigma_a^{-1} \circ \sigma_a)(n) = n.
    \end{equation}
    
    Para (2), seja $a \in Z(\phi)$ e $n \in [s]$. Então $\phi(a) = id$, e assim \begin{equation}
        \phi(n | a) = \phi(a)(n) = id(n) = n.
    \end{equation}
    
    Para (3), tome $n \in [s]$ e $a, b \in A$. Denote por $\sigma_a$ e $\sigma_b$ as permutações $\phi(a)$ e $\phi(b)$, respectivamente. Como $\phi(A) \leq P_s$, temos que $\sigma_a \circ \sigma_b \in \phi(A)$, ou seja, existe $c \in A$ tal que $\phi(c) = \sigma_a \circ \sigma_b$. Assim, temos \begin{equation}
...
\begin{nota}
    Introduziremos terminologia para as propriedades da proposição anterior, respectivamente: \begin{enumerate}
        \item Diremos que o autômato em questão sempre tem \textbf{transição reversível};
        \item Todo $a \in Z(\phi)$ é chamado de \textbf{barulho}; 
        \item Diremos que o autômato é \textbf{transitivo}.
    \end{enumerate}
\end{nota}

\begin{definicao}[Linguagem] \label{def:linguagem}
    Seja $\automataAs{\phi}$ um autômato, $i \in [s]$ e $\emptyset \neq F \subseteq [s]$. A \textbf{linguagem} de $(\phi, i, F)$, que denotaremos por $L(\phi, i, F)$, é o conjunto \begin{equation}
        L(\phi, i, F) = \set{p \in \ainf : \phi(i \mid p) \in F}.
    \end{equation}
\end{definicao}

\begin{nota}
    Para toda $p \in \ainf$, denotaremos por $pL(\phi, n, F)$ o conjunto \begin{equation}
        pL(\phi, n, F) = \set { p + q : q \in L(\phi, n, F)},
    \end{equation}
    onde $+$ é a concatenação no monóide livre sobre $A$. Em particular, se $p = a \in A$, concatenamos apenas uma letra, prefixando toda palavra da linguagem $L(\phi, n, F)$ por $a$.
\end{nota}

\section{Não são Conclusões}
