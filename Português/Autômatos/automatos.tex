...
\begin{nota}
    Seja $\automataAs{\phi}$ um autômato e $L(\phi, n, F)$ uma linguagem. Para toda $p \in \ainf$, denotaremos por $pL(\phi, n, F)$ o conjunto \begin{equation}
...
% \begin{proposicao}
%     Seja $\automataAs{\phi}$ um autômato e $F = {k}$. Se existe $a \in A$ tal que, para todo $i, j \in [s]$ com $i \neq j$ tivermos \begin{equation}
%         \emptyset \neq L(\phi, j, F) = aL(\phi, i, F) \neq \emptyset,
%     \end{equation}
...
% \begin{demonstracao}
%     Tome $p \in L(\phi, j, F)$. Temos que \begin{equation}
%         \phi(j | p) = k
%     \end{equation}
%     e que $ap \in L(\phi, i, F)$. Por definição, \begin{equation}
%         \phi(i | ap) = \phi(\phi(i | a) | p) = k.
%     \end{equation}
%     Disso, $\phi(j | p) = \phi(\phi(i | a) | p)$ e portanto $\phi(i | a) = j$.
% \end{demonstracao}

\section{Comentários, Notas, e Conclusões}
